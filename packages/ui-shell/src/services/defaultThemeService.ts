...
export function createDefaultThemeService(): ThemeService {
  return {
    getSettings(): ThemeSettings {
      ...
      const storedPrimaryColor = localStorage.getItem(STORAGE_KEY_PRIMARY_COLOR)
    ...
    saveSettings(settings: ThemeSettings): void {
      localStorage.setItem(STORAGE_KEY_DARK_MODE, JSON.stringify(settings.darkMode))
    ...
    applyTheme(settings: ThemeSettings): void {
      ...
      root.style.setProperty('--primary-color', THEME_COLORS[settings.primaryColor])
    },
  }
}