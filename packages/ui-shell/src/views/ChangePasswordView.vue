...
function validateForm(): boolean {
    ...
    if (currentPassword.value && newPassword.value && currentPassword.value === newPassword.value) {
        $f.setFieldError("newPassword", "New password must be different from current password");
        isValid = false;
    }
        ...
        <Card>
            <template #content>
                <form @submit.prevent="handleChangePassword" class="space-y-6">
                    <Message v-if="success" severity="success" :closable="true" @close="success = false">
