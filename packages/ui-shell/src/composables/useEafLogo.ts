 ...
 */
export function useEafLogo(placement?: MaybeRefOrGetter<LogoPlacement | undefined>): UseLogoReturn {
  ...
  const logoSrc = computed<string | undefined>(() => {
    ...
    const currentPlacement = toValue(placement) ?? LogoPlacement.AUTHORIZED_LAYOUT_SIDEBAR;
...
function resolveLogoSrc(logo: EafLogo, placement: LogoPlacement): string | undefined {
  switch (placement) {
